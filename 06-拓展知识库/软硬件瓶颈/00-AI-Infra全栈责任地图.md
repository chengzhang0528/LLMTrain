...
> **学习导航**：承接 D07 的一次模型运行和 D10 的规模化训练；本课把应用、模型、框架、算子、运行时、集群和硬件串成两条完整链；完成后应能从一个性能现象定位应该向哪一层要证据。
...
同一个“慢”可能来自排队、分词、长 prompt、算子未融合、显存带宽、KV Cache 碎片、通信、存储或故障恢复。没有全栈地图，初学者容易把所有问题都归给模型参数量，或看到一个推理引擎名称就认为答案质量由它决定。
...
| 训练或推理引擎 | batch、请求、缓存、并行配置 | 调度、连续批处理、分页缓存、检查点 | TTFT、TPOT、吞吐、重算与 OOM |
...
4. decode 每轮新增 1 个 token，并读取权重与历史 KV，主要影响 TPOT。
...
| 现象：TTFT 变差 | 区分证据 | 责任层 |
|---|---|---|
| 请求到 GPU 前已经等待很久 | 队列时间、并发、限流记录 | 应用与服务调度 |
| 输入突然变长 | prompt 长度分桶、prefill kernel 时间 | 模型计算与引擎 |
| tokenizer 或检索耗时上升 | CPU profile、检索耗时、网络记录 | 应用与运行时 |
| 多卡 prefill 同步等待 | 通信时间、拓扑、最慢 rank | 集群与硬件 |

只看“GPU 利用率 60%”无法区分这四种原因。正确问题是：时间花在哪一段，哪一层持有什么可反驳证据？

## 预设案例观察

课程提供一条冻结请求时间线，包含排队、tokenize、prefill、decode、通信和流式返回时间。学习者先标出每段责任层，再从候选优化中删掉无法解释当前证据的方案，不需要启动服务或运行 profiler。
...
资料启发与版本边界见 [AIInfraGuide 素材审计](../../07-来源与质量审计/AIInfraGuide-素材审计.md)。下一课：[指标与容量账本](01-指标与容量账本.md)
