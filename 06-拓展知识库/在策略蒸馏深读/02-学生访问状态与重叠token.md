...
论文还比较了 sampled-token 和不同 k 的 top-k 版本。按学生分布随机抽取一个 token 的估计在这些实验中已能接近多 token 支持；始终只取 argmax 的 Top-1 更不稳定。关键区别不是“一个 token 一定不够”，而是按分布抽样可以在许多位置和训练步上无偏覆盖高概率区域，argmax 则长期挤在单一模式上。
