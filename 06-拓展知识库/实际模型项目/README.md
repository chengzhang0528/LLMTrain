...
学习导航：承接基础闭环和方法选择；本路线通过预设项目档案，从真实问题反推模型、数据、系统和评测。学习者审查决策和证据，不实际构建、训练或部署系统。
...
| 领域任务小模型 | 分类、抽取、路由、受限生成 | 小基座 + 冻结任务集 | 靠参数记住持续变化的全部事实 |
...
目标是把工单分到固定类别，并识别“信息不足”。先比较关键词规则、Embedding 分类器和小型 Transformer；只有基线不足时才微调生成模型。验收重点是各类别召回率、拒识率、延迟和模型大小，而不是回答是否像聊天机器人。
...
先证明较强教师在冻结任务集上有效，再选择可部署学生。比较原始学生、回答蒸馏、logit/特征蒸馏（可获得时）、量化和最终服务版本。质量、延迟、吞吐、显存和成本使用同一批输入测量。

## 统一课程单元

1. [问题合同与冻结验收集](01-问题合同与冻结验收集.md)：先定义用户问题、失败代价和测试证据。
