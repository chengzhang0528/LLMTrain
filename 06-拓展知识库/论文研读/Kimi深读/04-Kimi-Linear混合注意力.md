...
线性注意力把历史压进状态 $S_t$。为了只表达“保留旧状态，再写入修正量”，可用下面的抽象式：

$$S_t=G_t\odot S_{t-1}+\Delta S_t.$$

其中 $G_t$ 控制旧状态各部分保留多少，$\Delta S_t$ 是当前 token 产生的修正量。KDA 的真实 delta 更新还会沿当前键方向擦除旧预测再写入新值，不能把本式当成论文公式；完整拆解见[Kimi K3：KDA 与混合注意力](06-Kimi-K3技术报告/02-KDA与混合注意力.md)。这样读历史不必保存每个 token 的完整 KV，但压缩状态可能丢失精确顺序和少数事实。MLA 层保留逐 token 的压缩潜变量，以支持精细的全局访问，像在一本压缩笔记旁边放少量可检索原页。混合比例、层位置和门控方式是模型设计的一部分，不能把 KDA 等同于“无损注意力”。
