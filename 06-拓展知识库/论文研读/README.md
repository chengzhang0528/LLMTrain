...
学习导航：论文主线有两种入口：按研究问题横向比较，或按模型系列连续学习。选定系列后，系列路线、完整目录和逐篇课程都在同一处，不需要在两套目录之间切换；跨系列材料另放在专题区。
...
| 想看一个模型家族怎样演进 | [按模型系列学习](#按模型系列学习) | 系列路线、完整目录和逐篇课程 |
| 已经知道要读哪篇 | [全部论文与学习进度](01-论文库.md) | 搜索论文并直接进入站内课程或原文 |
...
先进入一个系列的路线页，理解推荐顺序；需要查漏时打开完整目录；随后直接进入同一系列下的逐篇课程。侧边栏也按这个顺序展开，不再把“系列学习”和“单篇课件”拆成两套入口。

| 系列 | 从这里开始 | 主线课数量 | 观察重点 |
|---|---|---:|---|
| GLM | [从空白填充到长任务 Agent](04-GLM系列演进.md) | 5 | 预训练目标、规模化、对话、Agent、稀疏注意力 |
| Kimi | [从长思维链到 K3](05-Kimi系列演进.md) | 6 | 强化学习、长上下文、MoE、Agent、多模态 |
| DeepSeek | [从开放基座到推理与视觉](06-DeepSeek系列演进.md) | 6 | MoE、MLA、训练系统、推理 RL、视觉生成 |
| Qwen | [从多语言基座到 Omni](07-Qwen系列演进.md) | 6 | 通用主干、代码、视觉、思考模式、实时多模态 |

四条主线共 23 篇课程。路线页回答“为什么按这个顺序”，逐篇课程回答“这篇具体解决了什么”。完整目录还保留分支论文、模型卡和发布说明，但不会把它们伪装成主线课。
...
Kimi K3 已归入[Kimi 单篇论文课](Kimi深读/06-Kimi-K3技术报告.md)。报告较长，所以在该论文内部继续展开六个子课，不再另设顶层专题入口。
