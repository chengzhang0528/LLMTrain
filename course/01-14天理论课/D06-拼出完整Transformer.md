...
> **学习导航**：[模型架构总纲](模型架构总纲.md) -> D05 Attention 子层 -> **D06 组装完整模型** -> D07 运行模型；本课把表示、Attention、MLP、残差与归一化装成 Decoder-only 主干。完成后应能从模型定位到 Block、子层和参数矩阵，并沿形状从 token ID 走到 logits。

## 先记四层结构

1. **模型级：Embedding → 多个 Block → 最终 Norm → LM Head。**
2. **Block 级：Attention 负责跨位置汇总，MLP 负责逐位置特征变换。**
3. **连接级：残差保留主通路，归一化控制数值尺度。**
4. **形状级：序列轴通常贯穿主干，LM Head 才把特征轴映射到词表轴。**
