...
简单 RNN 把新状态写成“当前输入贡献 + 旧状态贡献”再过非线性，并在时间步之间共享同一组参数。

| 拿掉什么 | 立即发生什么 |
|---|---|
| 旧状态项 | 每一步只看当前输入，时间依赖消失 |
| 当前输入项 | 状态只能自行递推，无法吸收新观测 |
| 时间共享参数 | 不同长度需要不同参数，不能自然处理任意步数 |
| 非线性 | 状态递推退化为线性动力系统，表达范围改变 |

反复乘状态雅可比解释了梯度可能指数衰减或增长，但具体是否发生取决于权重、激活和数据。
