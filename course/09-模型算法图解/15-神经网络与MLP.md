...
MLP 用带偏置的线性层混合输入坐标，再用非线性打破“多层仍可合并成一层”的限制。

| 拿掉什么 | 立即发生什么 |
|---|---|
| 权重矩阵 | 各输入特征不能形成可学习组合 |
| 偏置 | 单个线性映射被限制为经过原点；有些架构会有意省略 |
| 所有层间非线性 | 多个线性映射可合并为一个矩阵，深度不再增加这类表达能力 |
| 输出层 | 隐藏特征没有映射到任务需要的维度或分数 |

非线性让表达集合扩大，但不证明层数越多越好；优化、数据与泛化仍需实验。
