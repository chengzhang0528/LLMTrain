...
GRPO 先在**同一提示的一组回答**中建立相对基线，再用相对优势调节策略概率；同题分组减少了不同问题固有难度对比较的干扰。

| 拿掉什么 | 立即发生什么 |
|---|---|
| 同题候选组 | 奖励差混入题目难度，不能再解释为同一问题中的相对表现 |
| 组内中心化或基线 | 原始奖励整体偏移会直接改变更新信号，方差与尺度含义变化 |
| 可核验奖励 | 算法仍会优化给定数字，但数字不再可靠代表任务完成 |
| 策略约束 | 策略可能为追奖励远离参考行为；是否失稳必须由日志与实验判断 |

组相对信号不等于绝对正确率；一组回答全错时，“组内最好”仍可能是错答案。
