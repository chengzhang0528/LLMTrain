...
这里不做公式推导，不要求手算，也不把一张图冒充完整实现。每章用图讲信息流、生命周期位置和同类选择，再用“采用判断”表明确优势、局限、适用条件、拒绝条件与采用后检查。完整定义、训练证据和工程限制仍由对应正式课程承担。
...
SFT、PPO 与 RLHF 见 [D11](../01-14天理论课/D11-SFT、LoRA与QLoRA.md) 和 [D12](../01-14天理论课/D12-对齐、强化学习与评测.md)，KV Cache 与执行优化见 [D07](../01-14天理论课/D07-模型一次运行到底发生什么.md) 和 [软硬件瓶颈](../06-拓展知识库/软硬件瓶颈/)。它们没有被遗漏，只是不在本目录重复拆章。
