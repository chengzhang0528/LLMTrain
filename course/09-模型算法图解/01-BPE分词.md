...
BPE 每次只合并当前语料中高频的**相邻**单元，并把合并顺序保存为确定规则。相邻约束保证新单元仍对应原序列中的连续片段。

| 改掉什么 | 立即发生什么 |
|---|---|
| 不要求相邻 | 会把原文中隔开的片段拼成一个 token，解码不再是原顺序的直接逆过程 |
| 不保存合并顺序 | 同一字符串在训练与推理时可能切分不同，已有 Embedding 行无法稳定对应 |
| 只保留完整词 | 新词、拼写变化和长尾字符串容易退回未知项；字节或字符回退就是补这条覆盖边界 |

高频合并证明的是词表与序列长度之间的一种折中，不证明切分符合词义，也不证明下游质量一定更高。
...
能说出“先学片段规则，再重复使用规则；片段不是知识”即可。继续深入看 [D08：从原始文本到训练样本](../01-14天理论课/D08-训练数据与分词器.md)。
