# GRU：用两类门合并控制旧状态与新信息

> **看图目标**：看完能说明更新门和重置门分别控制什么，并看出 GRU 与 LSTM 是同一递归单元位置的不同设计。

## 为什么需要它

GRU 也想解决简单 RNN 难以保留长距离信息的问题，但把 LSTM 的细胞状态与隐藏状态合并，并用较少的门控制状态更新。

## 主图

```mermaid
flowchart TB
  X["当前输入"] --> R["重置门：候选参考过去的比例"]
  H0["旧隐藏状态"] --> R
  H0 --> RH["旧状态 × 重置门"]
  R --> RH
  X --> N["由当前输入计算候选状态"]
  RH --> N
  X --> Z["更新门：旧状态保留比例"]
  H0 --> Z
  H0 --> KEEP["旧状态 × 更新门"]
  Z --> KEEP
  N --> WRITE["新候选 × 互补比例"]
  Z --> WRITE
  KEEP --> H1["两部分相加：新隐藏状态"]
  WRITE --> H1
  H1 --> NEXT["新隐藏状态传给下一步"]
```

重置门先缩放候选计算能参考的旧状态，候选状态再由当前输入和这部分过去共同产生。更新门给旧状态与新候选分配互补比例；两部分相加才得到新隐藏状态。门提供控制比例，不是候选内容的来源。

## 为什么不是随便设计的

本页采用“更新门表示保留旧状态”的常见写法：新状态是旧状态与候选状态按互补权重混合。另一些资料把同名门写成新候选权重，二者互为 `1-z`，必须看公式。

| 把门固定为什么 | 结构立即退化成什么 |
|---|---|
| 更新门 `1` | 完全保留旧状态，不写新候选 |
| 更新门 `0` | 完全采用新候选，旧状态只可能通过候选计算间接影响 |
| 重置门 `0` | 候选状态忽略旧状态，只根据当前输入形成 |
| 重置门 `1` | 候选完整参考旧状态，接近普通递归候选计算 |

互补权重保证逐坐标混合，而不是无界相加；这不证明 GRU 一定优于 LSTM。

## 对照图

```mermaid
flowchart TB
  A["门控递归网络"] --> L["LSTM"]
  L --> L1["独立细胞状态 + 三类门"]
  L1 -. "同位对照" .-> G["GRU"]
  G --> G1["单一隐藏状态 + 两类门"]
  G1 --> E["都沿时间递归，不能仅凭门数判断效果"]
```

虚线表示同位替代，不是 LSTM 先变成 GRU。GRU 通常更紧凑，但“参数更少”不自动等于“更快”或“效果更好”。

## 生命周期位置

```mermaid
flowchart TB
  A["设计：选择 GRU 递归单元"] --> B["训练：学习更新门和重置门"]
  B --> C["部署：逐步维护一个隐藏状态"]
  C --> D["流式或序列任务输出"]
  A:::focus
  B:::focus
  C:::focus
  classDef focus fill:#dff4e8,stroke:#24704a,color:#143d2b
```

GRU 是架构选择；它和 LSTM 通常不会在同一层按生命周期先后各跑一次。

## 同一位置的不同选择

| 递归单元 | 主要结构差异 | 常见效果与代价 |
|---|---|---|
| 简单 RNN | 无显式门控 | 参数少，长依赖较弱 |
| LSTM | 独立细胞状态与三类门 | 控制细，状态和参数更多 |
| GRU | 合并状态与两类门 | 结构紧凑，效果需按任务验证 |

## 采用判断

| 采用前后要问 | GRU 的答案 |
|---|---|
| 主要优势 | 比 LSTM 更紧凑地提供门控状态更新，常适合资源受限的递归序列模型 |
| 主要局限 | 状态控制更合并，仍受串行计算和固定状态限制；参数较少不保证更快或更准 |
| 适合考虑 | 需要门控递归但希望降低模型和状态复杂度，并能与 LSTM 做同条件比较时 |
| 不适合直接采用 | 仅按门的数量或参数量作决定，尚未测量目标任务质量和硬件单步延迟时 |
| 采用后必须检查 | 与简单 RNN、LSTM 的质量差异、长跨度遗忘、状态重置、梯度和真实设备延迟 |

## 看图复述

遮住正文，只看三张图回答：

1. 更新门混合哪两部分？
2. 重置门主要影响旧状态的哪次使用？
3. GRU 与 LSTM 是先后阶段还是替代选择？

能说出“GRU 用两类门维护单一隐藏状态，与 LSTM 同位竞争”即可。

## 方法边界

- 图省略具体方程、逐元素运算、双向结构与多层堆叠。
- 图中的门控缩放与最终相加都是逐元素运算。不同资料可能把更新门的数值解释为“保留旧状态”或“写入新候选”，两种写法互为补数；判断时应看公式，不只看符号名。
- 不同框架还可能在门的排列和候选计算细节上存在差异。
- 选择 GRU 仍需比较任务质量、训练速度、推理延迟和硬件实现。
