...
AdamW 的一阶移动统计平滑方向，二阶统计按近期梯度平方调整各坐标尺度，`ε` 避免分母过小，权重衰减则直接作用于参数而不是混进自适应梯度。

| 拿掉什么 | 更新规则退化成什么 |
|---|---|
| 一阶统计 | 不再平滑近期方向，更接近只看当前梯度的自适应更新 |
| 二阶统计 | 不再按坐标的近期幅度归一化，失去 Adam 的主要尺度适配 |
| `ε` | 二阶统计为 0 或极小时可能除零或产生异常大更新 |
| 训练早期的偏差校正 | 从 0 初始化的移动平均被系统性压小；影响大小取决于步数和系数 |
| 解耦权重衰减 | 变回另一种正则化/更新约定，不再是 AdamW 的定义 |

这些是公式结构的直接后果；AdamW 是否优于 SGD 仍需同数据、同预算和调度下比较。
...
能说出“梯度进入两类历史状态，权重衰减单独处理”即可。继续深入看 [D09：一个训练 Step 如何推动整次任务](../01-14天理论课/D09-训练任务内部的一次完整循环.md)。
