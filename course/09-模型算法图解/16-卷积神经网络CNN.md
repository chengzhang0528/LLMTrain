...
卷积把同一组核权重滑过不同位置，因此同时编码局部连接与参数共享。

| 改掉什么 | 立即发生什么 |
|---|---|
| 局部感受野 | 每个输出读取全部输入，退回更像全连接层的连接成本 |
| 跨位置权重共享 | 每个位置学习独立核，参数随空间大小增长，平移等变结构被破坏 |
| padding | 边缘输出尺寸缩小或边界信息处理改变；不一定是错误，而是明确设计选择 |
| stride | 固定为 1 时不再下采样；步幅过大可能跳过细节 |

参数共享带来结构先验，不保证对旋转、尺度变化或所有平移都天然不变。
