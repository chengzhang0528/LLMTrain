...
候选状态负责准备新内容；输入门只决定写入比例，不会自己生成内容。遗忘门控制旧细胞状态保留多少，输出门控制变换后的新细胞状态暴露多少。“门”输出的是连续比例，不是硬开关，也不是人工写好的规则。

## 为什么不是随便设计的

LSTM 的核心状态更新是“旧细胞状态乘遗忘门 + 新候选乘输入门”。加法通路让状态不必每步都被完整非线性重写。

| 把门固定为什么 | 结构立即退化成什么 |
|---|---|
| 遗忘门 `0` | 每步清空旧细胞状态，长程保留路径被切断 |
| 遗忘门 `1` | 旧状态不主动遗忘，若持续写入可能累积无关信息 |
| 输入门 `0` | 不写入新候选，状态无法学习当前输入的新内容 |
| 输出门 `0` | 细胞状态仍可内部传递，但本步隐藏输出被屏蔽 |

门接近 1 可让梯度沿细胞状态通路较直接传播，但不能保证永不遗忘或无限容量。
...
- 图中的“乘”都是逐元素缩放，“相加”也是逐元素相加，不是把整条向量压成一个数。
