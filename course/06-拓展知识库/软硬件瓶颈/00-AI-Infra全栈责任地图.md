...
> **学习导航**：承接 D07 的一次模型运行和 D10 的规模化训练；本课把应用与产品、模型架构与解码、模型资产与数值表示、框架与算子、编译与运行时、训练与推理引擎、集群系统和硬件串成两条完整链；完成后应能从一个性能现象定位应该向哪一层要证据。

## 本课核心判断

**同一个“慢”可能来自排队、输入、算子、带宽、缓存、通信或恢复；不能只归因于参数量，也不能把引擎当成知识来源。**

**性能结论必须绑定软件版本、硬件、精度、输入输出长度、并发和请求分布。** 理论 FLOPs、标称算力或单个 tokens/s 都不能独立证明端到端更快。
...
| 模型架构与解码 | token、隐藏状态、参数 | attention、MLP、MoE、解码与候选接受规则 | logits、FLOPs、激活与 KV 规模 |
| 模型资产与数值表示 | 权重、激活、KV Cache 的精度与格式 | 校准/训练、导出、低精度表示与质量回归 | 模型体积、量化误差、格式兼容性 |
| 框架与算子 | 张量和计算图 | 选择 kernel、自动求导、算子融合 | kernel 时间、图中断、数值正确性 |
| 编译与运行时 | 图、形状、设备资源 | 图编译、内存规划、有效精度、kernel 启动和数据搬运 | 启动开销、峰值内存、CPU/GPU 等待 |
| 训练与推理引擎 | batch、请求、缓存、并行配置 | 调度、连续批处理、分页缓存、检查点 | 引擎侧 TTFT/TPOT、吞吐、重算与 OOM |
| 集群系统 | 多卡任务、网络、存储 | 集合通信、拓扑放置、容错和恢复 | 通信占比、尾延迟、失败恢复时间 |
| 硬件 | kernel、张量和数据流 | 计算、HBM 读写、互联和存储 I/O | 峰值算力、带宽、容量、能耗与温度 |

每层都只能解释自己负责的部分。模型架构能说明理论计算量，数值表示能说明量化格式与误差，但二者都不能单独解释真实 P99；硬件标称算力能给出上限，却不能证明软件已经生成合适 kernel。
...
2. 服务端通常在 CPU 上完成 tokenizer；推理引擎决定何时接纳请求。具体实现也可使用其他处理器或把部分处理并入服务链路。
3. prefill 处理 4000 个输入 token，建立各层 KV Cache，通常是引擎侧首 token 延迟的重要组成；若从应用端开始计时，TTFT 还要加上排队、检索、模板和网络等时间。
...
| 输入突然变长 | prompt 长度分桶、prefill kernel 时间 | 应用与产品 / 模型架构与解码 / 训练与推理引擎 |
...
| prefill | 138 ms | GPU kernel 时间占主导 | 模型架构与解码 / 框架与算子 / 硬件 |
| 多卡通信 | 26 ms | 最慢 rank 等待 9 ms | 集群系统 / 硬件 |
| 稳态 decode | 首 token 后 199 个间隔共约 4.78 s | 引擎侧平均 TPOT 为 24 ms | 模型架构与解码 / 训练与推理引擎 / 硬件 |
...
4. 一次长 prompt 请求的 TTFT 上升，至少要检查哪些时间段？
