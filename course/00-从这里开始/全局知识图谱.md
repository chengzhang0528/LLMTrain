  ...
  "modes": [
    {
      ...
      "rebuild": ["input", "tokenize", "embedding", "position", "block", "cache", "logits", "sample", "output"],
      "nodes": [
        { "id": "input", "label": "文本 / 图像 / 语音", "shape": "原始模态", "kind": "input", "visual": "modalities", "visualMeaning": "入口片段表示不同输入类型，不共用一组参数点。", "owner": "应用与模态编码器" },
        ...
        { "id": "sample", "label": "解码 / 采样", "shape": "(B,V) → (B,)：每条活跃序列 1 个 ID", "kind": "choose", "visual": "selection", "visualMeaning": "每条活跃序列的候选分数槽位与一个接收标记，表示选择过程，不表示多条文本分支。", "owner": "解码代码" },
      ...
      "edges": [
        ...
        { "id": "e2-embedding", "from": "tokenize", "to": "embedding", "label": "ID 查表" },
        { "id": "e3", "from": "position", "to": "block", "label": "按架构接入位置线索" },
        ...
        { "id": "e7", "from": "sample", "to": "output", "label": "应用处理后可选输出" },
        { "id": "e8", "from": "sample", "to": "position", "label": "未停止才加入新位置" }
      ],
      "steps": [
        ...
        { "title": "查表并进入多层前向", "watch": "token ID 查 Embedding 得到 (B,T,D) 初始表示；位置线索再按具体架构接入，生成时 Block 还会读写各层 KV Cache。", "purpose": "让离散符号和顺序线索进入矩阵、注意力和 MLP 的连续计算。", "detail": "位置 Embedding 可在 Block 前与 token 表示结合；RoPE 在注意力内部作用于 Q/K；相对位置偏置作用于注意力分数。三者不能统一画成向 Embedding 相加。KV Cache 是前向的输入输出状态，不会脱离模型自行产生 logits；普通推理读取权重，不执行反向传播。", "reflection": "为什么不能把 RoPE 画成 token ID 查表之后再加一个位置向量？", "active": ["tokenize", "e2-embedding", "embedding", "position", "e3", "e4", "block", "e5-cache", "cache", "e5-reuse"], "shape": "(B,T,D) + 按架构接入的位置线索 + 各层历史 K/V" },
        ...
        { "title": "选择、应用处理并循环", "watch": "解码代码为每条活跃序列接受 1 个 ID；应用处理后满足条件才可能反分词显示，未停止才获得新的序列位置并进入下一轮前向。", "purpose": "形成可见回答，同时解释流式输出为何看起来像连续写作。", "detail": "EOS、协议控制 token、停止序列的一部分或尚未组成完整字符的字节片段，都可以使本轮产生 0 个可见字符。基础路径通常逐 token 接受；推测解码等实现可以批量验证并接受多个。模型权重通常静态，KV Cache 是前向读写的请求状态，推理引擎负责调度和缓存。", "reflection": "已接受的 ID 为什么可能不显示，只有未停止的序列才会进入下一轮？", "active": ["logits", "e6", "sample", "e7", "output", "e8", "position"], "shape": "(B,V) → (B,) → 可选输出；未停止才进入下一轮前向" }
      ]
    },
    {
      "id": "training",
...
离策略蒸馏学习固定教师轨迹；在策略蒸馏则在学生访问的状态上请求教师信号。后者更贴近学生分布，但仍要检查教师是否带来新增能力、高概率 token 是否重合、长轨迹尾部监督是否退化。见[教师越强越好吗](../06-拓展知识库/在策略蒸馏深读/01-教师越强越好吗.md)。
...
第一次进入课程，直接看[从这里开始](./)；已经观察到具体问题时，再用[学科地图](学科地图.md)定位责任层。

已有问题时，可直接进入[实际模型案例](../06-拓展知识库/实际模型项目/01-问题合同与冻结验收集.md)、[模型评测与选型](../06-拓展知识库/模型评测与选型/)、[幻觉与可靠性](../06-拓展知识库/幻觉与可靠性/01-生成为什么不保证事实.md)、[模型后训练](../06-拓展知识库/模型后训练/01-基座审计与任务边界.md)、[多模态基础](../06-拓展知识库/多模态基础/01-从模态到张量.md)或[软硬件瓶颈](../06-拓展知识库/软硬件瓶颈/00-AI-Infra全栈责任地图.md)。
